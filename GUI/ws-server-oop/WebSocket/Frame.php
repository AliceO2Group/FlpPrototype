...
class Frame {
	...
	/// Constructs empty frame to be filled with data comming from socket or creates frame based on Opcode, Payload
	/// \param $opcode
	/// \param $payload
	...
	/// \param $rawheader data from the socket
	...
	public function controlFrame() {
			if ($this->getRsv() != 0) {
				echo $this->frameDump();
				throw new WebSocketException("RSV must be 0", 1002);
			}
			switch ($this->getOpcode()) {
				case Frame::OPCODE_CNT: /* last continuation frame, fin must be 1 */
					throw new WebSocketException("Continuation frames are not supported yet");
				break; 
				case Frame::OPCODE_TXT: /* text frame - 0x1 */		
					break;
				case Frame::OPCODE_BIN: /* binary frame - 0x2; NOT SUPPORTED */
					throw new WebSocketException(sprintf("Binary frame is not supported: [%d]", $this->getOpcode()));
					break; 
				case Frame::OPCODE_CLS: /* closing frame - 0x8 */
					break;
				case Frame::OPCODE_PING: /* ping - 0x9*/
					if ($this->getLength() > 125) {
						throw new WebSocketException("Ping frame with payload of more than 125 bytes");
					}
					break;
				...
				default: /* reserveed: 0x3 - 0x7 and 0xB - 0xF */ 
					throw new WebSocketException("Unknows WS frame");
			}
	}
