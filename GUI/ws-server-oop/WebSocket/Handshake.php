...
class Handshake {
	...
	/// \see http://tools.ietf.org/html/rfc6455#page-14
	/// \throws WebSocketException
	/// \throws OriginException
	...
	protected function httpError($message = "No details", $code = 404) {
		...
		switch ($code) {
			case self::ERROR_BAD_REQUEST: // 400
				$response .= self::HTTP_PROTOCOL . ' 400 Bad Request' . "\r\n";
				$response .= self::HEADER_VERSION . ': 13';
			break;
			case self::ERROR_FORBIDDEN: // 403
				$response .= self::HTTP_PROTOCOL . ' 403 Forbidden' . "\r\n";
			break;
			case self::ERROR_NOT_FOUND: // 404
				$response .= self::HTTP_PROTOCOL . ' 404 Not Found' . "\r\n";
			break;
			case self::ERROR_UPGRADE_REQUIRED: // 426
				$response .= self::HTTP_PROTOCOL . ' 426 Upgrade Required' . "\r\n";
			break;	
		}
